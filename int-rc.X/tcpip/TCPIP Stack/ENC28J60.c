...
typedef struct  __attribute__((aligned(2), packed))
{
    ...
    static inline __attribute__((__always_inline__)) void WaitForDataByte( void )
    {
		...
		Nop(); // TODO this delay is necessary with O2, but not with no optimisation. This might because of the CS hold time requirement during writes. Check this.
		Nop();
		Nop();
		Nop();
		Nop();
...
void MACInit(void)
{
	...
	ENC_SPISTAT = 0;        // clear SPI
		...
		ENC_SPICON1 = 0x00F;     // 1:1 primary prescale, 5:1 secondary prescale (8MHz  @ 40MIPS)
